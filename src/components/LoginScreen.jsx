...
export default function LoginScreen({ onRecoveryClick }) {
  ...
  return (
    <div className="relative overflow-hidden py-4 px-4 sm:py-6 sm:px-6 lg:py-8 lg:px-6 text-center min-h-0 overflow-y-auto">
      ...
      <div className="relative z-10">
        <div className="relative inline-block mb-3 sm:mb-4">
          ...
          <img
            ...
            className="h-[140px] w-[140px] sm:h-[180px] sm:w-[180px] lg:h-[220px] lg:w-[220px] rounded-full object-cover shadow-xl drop-shadow-2xl animate-logo-float"
          />
        </div>
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-wider uppercase bg-gradient-to-r from-sky-400 to-sky-600 bg-clip-text text-transparent mb-1">
          Frota do Atlântico
        </h1>
        <p className={`${mutedClass} text-[10px] sm:text-xs uppercase tracking-widest font-medium mt-1`}>
          Navegamos com Confiança
        </p>

        <form onSubmit={handleLogin} className="w-full max-w-[320px] mx-auto mt-4 sm:mt-6 text-left px-1">
          ...
          <div
            ...
            className="mt-6 sm:mt-10 flex flex-col items-center gap-2 sm:gap-3 cursor-pointer"
            aria-label="Toque para entrar"
          >
            <div
              className={`relative flex h-[90px] w-[90px] sm:h-[110px] sm:w-[110px] items-center justify-center rounded-full border-2 border-sky-400/40 bg-gradient-to-b from-sky-400/15 to-transparent shadow-lg transition overflow-hidden ${
                scanning ? 'ring-4 ring-sky-400/30' : 'hover:border-sky-400 hover:shadow-sky-500/20'
              }`}
            >
              {scanning && (
                <div
                  className="absolute h-[70px] w-[70px] sm:h-[85px] sm:w-[85px] rounded-full bg-gradient-to-b from-transparent via-sky-400/80 to-transparent animate-scan-finger blur-[2px]"
                  aria-hidden
                />
              )}
              <Fingerprint className="h-12 w-12 sm:h-16 sm:w-16 text-sky-400 relative z-10" strokeWidth={2} />
            </div>
            <p className={`text-xs sm:text-sm font-medium tracking-wide ${mutedClass}`}>Toque para entrar</p>
          </div>
          {onRecoveryClick && (
            <button
              type="button"
              onClick={onRecoveryClick}
              className={`mt-4 sm:mt-6 text-xs sm:text-sm hover:text-sky-400 transition ${mutedClass}`}
            >
              Esqueceu PIN?
            </button>
          )}
